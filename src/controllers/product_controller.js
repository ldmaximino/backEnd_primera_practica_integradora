...
export const createProduct = async (req, res, next) => {
    try {
      ...
      if(productExist) return res.status(404).json({msg:'Product already exists'});
      const product = await service.createProduct(req.body);
      if(!product) return res.status(404).json({msg:'Error creating product'})
      return res.status(201).json(product);
    } catch (error) {
      next(error);
    }
...
export const updateProduct = async (req, res, next) => {
    try {
      const { pid } = req.params;
      const productExist = await service.getProductById({_id: pid});
      if(!productExist) return res.status(404).json({msg:'Product not found'});
      const product = await service.updateProduct(pid, req.body);
      if(!product) return res.status(404).json({msg:'Error updating product'})
      return res.status(201).json(product);
    } catch (error) {
      next(error);
    }
